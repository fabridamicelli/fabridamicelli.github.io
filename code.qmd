...
### [logger-bot](https://github.com/PyCampES/logger-bot)
`logger-bot` is a lean Telegram workout assistant that turns voice notes into database training logs.  
It's a small composed system: chat interface in front, ML extraction in the middle, and a queryable data store behind it.
...
On top of logging, it exposes lightweight command handlers for health checks, last-set lookup, and read-only SQL queries over your workout history.
For example you can run a query with a command like this:

![](/images/screenshot-sql.png){width=10cm fig-align="center"}
